...
async function handleExecCommand () {
  ...
  const findHosts = shell
    .grep('host', `${path.join(require('os').homedir(), '.ssh/config')}`)
    .replace(' ', '')
    .replace(/\r?\n|\r/g, '')
    .split('host')
  ...
  const availableHosts = ['local', ...findHosts];

  const { selectedHost } = await inquirer
    .prompt([{
      type: 'autocomplete',
      name: 'selectedHost',
      ...
      source: function (answsers, input) {
        return new Promise(resolve => {
          const fuzzyResult = fuzzy.filter(input, availableHosts);
          resolve(
            fuzzyResult.map(function (el) {
              return el.original || el;
            })
  ...
  if (selectedHost === 'local') {
    ...
    execute(`ssh ${selectedHost} '${getByName(findCommand, 'command')}'`);
  }
}
